...
import {ListHeader} from "@/app/(platform)/(dashboard)/board/[boardId]/_components/listHeader";
...
export const ListItem = ({data, index} : ListItemProps) => {
    ...
    return (
        <Draggable draggableId={data.id} index={index}>
            {(provided) => (
                <li {...provided.draggableProps} ref={provided.innerRef} className={"shrink-0 h-full w-[272px] select-none"} onChange={() => {}}>
                    <div {...provided.dragHandleProps} className={"w-full rounded-md bg-[#f1f2f4] shadow-md pb-2"}>
                        ...
                        <CardForm listId={data.id} ref={textAreaRef} isEditing={isEditing} disableEditing={disableEditing} enableEditing={enableEditing}/>
                    </div>
                </li>
            )}
        </Draggable>
